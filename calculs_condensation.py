...
def solve_conduit(T_e, T_u, T_uo, L, dm, D_h, Res_therm, alpha_a):
    """
    ...
    ----------
    T_e : pint.Quantity
        Température fumée sortie foyer (K ou °C)
    T_u : pint.Quantity
...
for i in range(4):
    ...
    Res_therm = 0 / (ureg.watt / (ureg.meter ** 2 * ureg.kelvin))
    res = solve_conduit(T_e=T_in, T_u=T_u, T_uo=T_uo, L=L, dm=dm, D_h=D_h, Res_therm=Res_therm, alpha_a=alpha_a_ext)
    print(json.dumps(res, indent=2, default=str))
